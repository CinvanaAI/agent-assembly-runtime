...
from agent_assembly_runtime.agent_schema import (
...
class AgentNormalizer:
    ...
    def normalize_agent(self, raw_agent: object, fallback_name: str = "") -> dict[str, Any]:
        ...
        require_known_keys(source=raw_agent, allowed_keys=CANONICAL_AGENT_KEYS, label="agent")
    ...
    def normalize_group(self, raw_group: object, group_index: int) -> dict[str, Any]:
        ...
        require_known_keys(source=raw_group, allowed_keys=CANONICAL_GROUP_KEYS, label=f"group entry {group_index}")
    ...
    def normalize_task(
        ...
        require_known_keys(source=raw_task, allowed_keys=CANONICAL_TASK_KEYS, label=f"task entry {task_index}")
    ...
    def normalize_workflow(self, raw_workflow: object, task_name: str) -> dict[str, str]:
        ...
        require_known_keys(source=raw_workflow, allowed_keys={"workflow_source"}, label=f"task '{task_name}'.workflow")
    ...
    def normalize_default_returns(self, raw_default_returns: object, label: str) -> list[dict[str, Any]]:
        ...
        for index, item in enumerate(raw_items, start=1):
            ...
            require_known_keys(source=item, allowed_keys=CANONICAL_DEFAULT_RETURN_KEYS, label=f"{label} entry {index}")
    ...
    def normalize_package_logic(
        ...
        require_known_keys(source=raw_logic, allowed_keys={"logic_source"}, label=f"package '{package_name}'.logic")
    ...
    def normalize_packages(self, raw_packages: object, task_name: str) -> list[dict[str, Any]]:
        ...
        for expected_position, item in enumerate(raw_items, start=1):
            ...
            require_known_keys(source=item, allowed_keys=CANONICAL_PACKAGE_KEYS, label=f"package entry {expected_position}")
    ...
    def normalize_replacements(self, raw_replacements: object, label: str) -> list[dict[str, str]]:
        ...
        for index, item in enumerate(raw_items, start=1):
            ...
            require_known_keys(source=item, allowed_keys={"to_be_replaced", "value"}, label=f"{label} entry {index}")
    ...
    def normalize_arguments(
        ...
        for index, item in enumerate(raw_items, start=1):
            ...
            require_known_keys(source=item, allowed_keys=CANONICAL_ARGUMENT_KEYS, label=f"argument entry {index}")
    ...
    def normalize_returns(
        ...
        for index, item in enumerate(raw_items, start=1):
            ...
            require_known_keys(source=item, allowed_keys=CANONICAL_RETURN_KEYS, label=f"return entry {index}")
    ...
    def normalize_prompts(self, raw_prompts: object, task_name: str) -> list[dict[str, str]]:
        ...
        for index, item in enumerate(raw_items, start=1):
            ...
            require_known_keys(source=item, allowed_keys=CANONICAL_PROMPT_KEYS, label=f"prompt entry {index}")
    ...
    def normalize_memory(self, raw_memory: object, task_name: str) -> list[dict[str, Any]]:
        ...
        for index, item in enumerate(raw_items, start=1):
            ...
            require_known_keys(source=item, allowed_keys=CANONICAL_MEMORY_KEYS, label=f"memory entry {index}")
